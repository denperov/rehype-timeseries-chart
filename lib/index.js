...
//   * containerClass  – CSS class on the wrapping <div> (default "timeseries-chart-container")
 ...
 * @param {Array<HastElement|{type: string; value: string}>} [children]
 * @returns {HastElement}
 */
...
const rect = (x, y, w, h, props = {}) =>
  el('rect', {x, y, width: w, height: h, ...props})
 ...
 * @param {number} w
 * @param {number} h
 * @param {Array<HastElement>} children
 * @returns {HastElement}
 */
 ...
 * @param {Array<{name: string; values: Array<{x: Date|number; y: number}>}>} series
 * @param {{isDate: boolean; formatter: (d: any) => string}} xInfo
 * @param {{width?: number; height?: number; title?: string; textColor?: string; backgroundColor?: string}} [opt]
 * @returns {HastElement}
 */
 ...
 * @param {Array<{x: Date|number; [key: string]: number}>} rows
 * @param {string[]} names
 * @returns {Array<{name: string; values: Array<{x: Date|number; y: number}>}>}
 */
const toSeries = (rows, names) =>
  names.map((n) => ({name: n, values: rows.map((r) => ({x: r.x, y: r[n]}))}))
